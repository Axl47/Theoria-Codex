...
- The app now accepts external Pixiv post URLs in `<scheme>://<pixiv-host>/<locale>/artworks/<id>` format (`http` or `https`; hosts: `www.pixiv.com`, `pixiv.com`, `www.pixiv.net`, `pixiv.net`; two-letter locale like `en`, `ja`) and opens them directly into Viewer after resolving the post in-app
