...
Before translating Japanese, conservatively join adjacent tall ML Kit blocks that form one compact vertically overlapping cluster. Read columns right-to-left, remove whitespace inside each vertical column, and render the union polygon as one tappable phrase. Do not split a compact cluster merely because a column ends in punctuation: manga bubbles commonly contain multiple punctuated columns. Never apply this heuristic to horizontal Japanese or Chinese/Korean, and prefer separate phrases when gaps, overlap, column widths, or combined aspect ratio are ambiguous; crossing speech bubbles is worse than leaving a sentence fragmented.

If the normal full-image Japanese pass returns no accepted region, retry only that recognizer over four overlapping crops upscaled within the 2048-pixel edge bound; run the same crops once more in high-contrast grayscale only if the first crop pass is also empty. Japanese recovery may hallucinate a Han prefix on a Latin watermark (observed `三RMARK`). Keep the stricter joined-uppercase noise check scoped to recovery crops, preserving normal mixed-script brand text and standalone CJK; do not replace it with a broad global script-ratio cutoff. Map crop geometry back into the original image, deduplicate only matching text with substantial spatial overlap, then apply vertical grouping. Do not multiply normal successful scans, apply this recovery to Chinese/Korean, or persist derived bitmaps.

Viewer translation uses the self-hosted batch gateway at `https://translate.axor.dev`, backed by Google Cloud Translation's standard NMT model under the project owner's written adult-content and attribution exception; do not use Android/OEM or ML Kit Translation. After accepted OCR, resolve every current-image phrase from the durable local cache, deduplicate misses, and start bounded background batches so taps reuse ready or in-flight work. A request may contain only those exact detected phrases, one `ja`/`zh-Hans`/`ko` source code, the `en` target, and text format. The gateway maps `zh-Hans` to Google's `zh-CN`. Never send image bytes, media URLs, post identity, taxonomy, headers, or account data. Require HTTPS, cap each phrase at 1,000 characters, each batch at 32 phrases/5,000 characters, and responses at 64 KiB; preserve route cancellation/stale-result rejection and surface failures only after the user taps a relevant region.

Durable Viewer translations live in Room's bounded `viewer_translation_cache` table, independently from posts and OCR analysis. Key them by backend version, source language, and SHA-256 of the exact normalized source phrase; store only the English result, expiry, and last-used time—never plaintext source text or image/post identity. Retain at most 1,000 entries for 90 days, and change the backend version when translation semantics change so old results miss naturally.

Google Play-services OCR modules are shared device capabilities and may already be present even when Theoria did not initiate a download. Settings readiness and its language toggles describe OCR availability only; translation has no per-device language-pack UI. The legacy-named `deploy/libretranslate/` path owns the batch-only Google gateway because Dokploy's service/domain identity still uses that name; no LibreTranslate protocol compatibility is required before the first release. Keep the Google API key only in the Dokploy environment, restrict it to the Translation API and server IP, send it through `X-Goog-Api-Key`, allow only one gateway batch at a time, and retain server-side request/phrase/batch/response limits plus the 15,000-character rolling daily budget. Never add a durable client secret to the open-source APK. Retain the written exception outside the repository and re-check it before changing how or where results appear. Samsung's `enja`, `enzh`, and `enko` packages belong to allowlisted OEM features and must not influence Theoria readiness.
