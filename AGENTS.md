...
Pixiv Ugoira loading publishes immutable decoded frame batches with the full metadata timeline. Keep playback bounded by the available frame end so buffering cannot skip time or complete a Viewer session early; never recycle a published bitmap on cancellation. Both playback and archive acquisition are reference-counted, and only the final departing consumer cancels shared work. Retain one full archive validation per acquisition, cancellable bounded frame reads, and complete-only decoded cache admission. ZIP timing sidecars use explicit JSON fields tied to archive length and modification time, so disk reuse requires no new reflected wire model or credentials request. Card/Viewer memory budgets may reduce decoded resolution for long animations; exports retain original resolution.
...
Last reading position is independent from highest-page progress. `ReadingPositionRepository` stores the last one-based media number by canonical post and Recents section; Start over resets only this position. Viewer persists the selected canonical ID and a bounded ordered ID window for cold restoration, preserving launch filtering/order instead of rebuilding a different feed by index. Creator navigation identity must be restorable before rendering its route. Viewer save callbacks carry the active launch context, never the already-consumed pending handoff.

Named saved searches are device-wide like Recents, bounded independently, and reuse `RecentSearchPayloadCodec` for exact grouped, Multi-Search, and FYP replay. History clearing and pruning never remove pins. Backup merges must preserve exact source order and source-specific FYP tags.
...
Profile backups are logical metadata snapshots, never copies of live Room/DataStore files. Exclude credentials, offline media, signed media URLs, and device paths while preserving canonical public provider links. Restore creates fresh profile/Codex/follow identities and uses a bounded fsynced intent journal; finish `recoverPendingRestore()` before exposing the app graph. Preserve existing shared post payloads and keep every phase idempotent. Legacy Likes may lack a full Post, and system Likes Codices may contain independently saved members; these are distinct valid states.

Completed offline media lives outside disposable caches with collection ownership. Clear/remove invalidates work before resolution begins, including queued automatic saves; same-post acquisitions retain an already-published generation so another owner cannot invalidate Viewer paths. Cache-on-save queues application-owned acquisition after membership commits and must not hold back success feedback/statistics. Keep offline-owned local paths out of durable shared Post/Recents payloads and strip stale owned paths before online fallback. Ugoira offline copies retain archive timing beside the ZIP. Clear disposable media through their owners, preserving active archives and pinned offline copies.

Offline manifests keep the authoritative source snapshot separately from the chosen download rendition. Restore source media URLs, MIME, gallery shape and media count before saving, liking, recording history or calculating duration identity; playback keeps the chosen local rendition. Provenance follows live returned media references through weak identity keys, including shallow Post copies and removal, and catalog scans must not populate that process-only registry.

Canonicalize both Viewer visibility and media-progress writes: the first progress callback can create a Recents membership before the visibility write finishes. A later sparse source update preserves existing local paths, so it cannot repair a playback rendition admitted by that earlier callback.
...
Compose test v2 `performClick` injects pointer input. Use `performSemanticsAction(SemanticsActions.OnClick)` to verify an accessibility click when a separate pointer gesture occupies the same node. Robolectric accessibility-state changes must target the manager from the screen's actual `LocalContext`, since an application-context shadow may have a different listener registry.
