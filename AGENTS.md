...
Viewer translation uses the self-hosted gateway at `https://translate.axor.dev`, backed by Google Cloud Translation's standard NMT model under the project owner's written adult-content and attribution exception; do not use Android/OEM or ML Kit Translation. OCR remains on-device, and a translation request may contain only the exact tapped phrase, its `ja`/`zh-Hans`/`ko` source code, the `en` target, and the text format. The gateway maps `zh-Hans` to Google's `zh-CN`. Never send image bytes, media URLs, post identity, taxonomy, headers, or account data. Require HTTPS, cap phrases at 1,000 characters and responses at 64 KiB, use bounded connection/read timeouts, preserve route cancellation/stale-result rejection, and surface server failures only in the retryable translation card.

Google Play-services OCR modules are shared device capabilities and may already be present even when Theoria did not initiate a download. Settings readiness and its language toggles describe OCR availability only; translation has no per-device language-pack UI. The legacy-named `deploy/libretranslate/` deployment retains the narrow LibreTranslate form/JSON contract for installed app compatibility but forwards the exact phrase and language pair to Google NMT. Keep the Google API key only in the Dokploy environment, restrict it to the Translation API and server IP, send it through `X-Goog-Api-Key`, allow only one gateway request at a time, and retain server-side request/character/response limits. Never add a durable client secret to the open-source APK. Retain the written exception outside the repository and re-check it before changing how or where results appear. Samsung's `enja`, `enzh`, and `enko` packages belong to allowlisted OEM features and must not influence Theoria readiness.
