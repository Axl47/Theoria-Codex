...
Viewer translation uses the self-hosted Hy-MT2 endpoint at `https://translate.axor.dev`; do not use Android/OEM or ML Kit Translation. OCR remains on-device, and a translation request may contain only the exact tapped phrase, its `ja`/`zh-Hans`/`ko` source code, the `en` target, and the text format. Never send image bytes, media URLs, post identity, taxonomy, headers, or account data. Require HTTPS, cap phrases at 1,000 characters and responses at 64 KiB, use bounded connection/read timeouts, preserve route cancellation/stale-result rejection, and surface server failures only in the retryable translation card.

Google Play-services OCR modules are shared device capabilities and may already be present even when Theoria did not initiate a download. Settings readiness and its language toggles describe OCR availability only; translation has no per-device language-pack UI. The legacy-named `deploy/libretranslate/` deployment retains the narrow LibreTranslate form/JSON contract for installed app compatibility but runs `tencent/Hy-MT2-1.8B-GGUF:Q4_K_M` through pinned llama.cpp plus a small gateway. Keep the model endpoint private, preserve the model volume, allow only one inference at a time, enforce the 2.25 GiB/three-CPU model limit and server-side request/character/response limits, and never add a durable client secret to the open-source APK. Samsung's `enja`, `enzh`, and `enko` packages belong to allowlisted OEM features and must not influence Theoria readiness.
