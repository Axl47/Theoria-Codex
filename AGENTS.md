...
Google Play-services OCR modules are shared device capabilities and may already be present even when Theoria did not initiate a download; never infer translation readiness from OCR availability. A language is Viewer-ready only when both its OCR module and its exact source-to-English on-device translation pair are present. On Samsung, detect the installed `enja`, `enzh`, and `enko` language-pack packages and send missing pairs to their language-specific Galaxy Store pages; keep Samsung's resolved `com.samsung.android.smartsuggestions.translate.settings.LAUNCH_SETTINGS` action as the general-settings fallback. Other Android 12+ devices use exact `STATE_ON_DEVICE` translation capabilities plus the standard translation settings intent. Do not request optional translation flags unless the device capability explicitly supports them.
