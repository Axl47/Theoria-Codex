...
Before translating Japanese, conservatively join adjacent tall ML Kit blocks that form one compact vertically overlapping cluster. Read columns right-to-left, remove whitespace inside each vertical column, stop joining after Japanese terminal punctuation, and render the union polygon as one tappable phrase. Never apply this heuristic to horizontal Japanese or Chinese/Korean, and prefer separate phrases when gaps, overlap, column widths, or combined aspect ratio are ambiguous; crossing speech bubbles is worse than leaving a sentence fragmented.
