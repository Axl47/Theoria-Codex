...
Never add `androidx.benchmark.junit4.SideEffectRunListener` to the personal-device benchmark lane. In AndroidX Benchmark 1.5.0-alpha07 it disables 41 unrelated packages, including Play Store and Google Play services, then unconditionally enables every package without restoring prior state. The benchmark APK instead isolates its application ID and storage and removes production deep links, App Links verification, install/network permission, and FileProvider. Artifact verification must prove those removals without changing any device package state.
