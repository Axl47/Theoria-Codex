...
Feed preview ExoPlayers are leased from the application-owned reusable slot pool, and lazy-grid `PlayerView`s opt into `AndroidView` reuse. Forgetting a card or leaving a route must detach its surface, remove the card listener, pause, and return the lease without synchronously calling `ExoPlayer.release()`; slow release is delayed and paced only after no preview lease is active. The idle-retention bound must never cap simultaneous visible autoplay. Muted feed players disable the audio track type so they do not allocate audio decoders merely to render silent previews. Keep player creation/release trace sections when changing this lifetime boundary so long-scroll and tab-switch acceptance can distinguish leasing from actual codec teardown.
