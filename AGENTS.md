...
Viewer translation uses the self-hosted LibreTranslate endpoint at `https://translate.axor.dev`; do not use Android/OEM or ML Kit Translation. OCR remains on-device, and a translation request may contain only the exact tapped phrase, its `ja`/`zh-Hans`/`ko` source code, the `en` target, and the text format. Never send image bytes, media URLs, post identity, taxonomy, headers, or account data. Require HTTPS, cap phrases at 1,000 characters and responses at 64 KiB, use bounded connection/read timeouts, preserve route cancellation/stale-result rejection, and surface server failures only in the retryable translation card.

Google Play-services OCR modules are shared device capabilities and may already be present even when Theoria did not initiate a download. Settings readiness and its language toggles describe OCR availability only; translation has no per-device language-pack UI. Keep `deploy/libretranslate/compose.yaml` restricted to `en`, `ja`, `zh`, and `ko`, persist its model volume, and retain server-side request/character limits because the personal open-source APK contains no durable client secret. Samsung's `enja`, `enzh`, and `enko` packages belong to allowlisted OEM features and must not influence Theoria readiness.
